...
```{r}
tab5.col5 <- lm(patient.years ~ stations + total.staff , data=dialysis)
tab5.col6 <- plm(patient.years ~ stations + total.staff , data=dialysis,index="provfs" )

tab5.col2 <- lm(patient.years ~ pct.septic + stations + total.staff, data=dialysis)
tab5.col3 <- plm(patient.years ~ pct.septic + stations + total.staff , data=dialysis,index="provfs" )
...
inc <- with(dialysis, stations > 0 & total.staff > 0 & net.hiring != 0 & !is.na(stations) & ! is.na(total.staff) & !is.na(net.hiring) & (net.investment == 0) & ! is.na(std.mortality))

iv1 <- ivreg(log(patient.months) ~ quality + poly(log(stations), log(total.staff),net.hiring,degree=2)*I(net.hiring > 0) | log(std.mortality + .01) + poly(log(stations), log(total.staff),net.hiring,degree=2)*I(net.hiring > 0), data=subset(dialysis,inc),na.action="na.exclude")
...
obj <- function(b){
  bx <- with(dialysis,log(stations)*b[1] + log(total.staff)*b[2])
